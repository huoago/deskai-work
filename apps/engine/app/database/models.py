...
class FileOrganizationBatch(Base):
    __tablename__ = "file_organization_batches"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    task_id: Mapped[str] = mapped_column(ForeignKey("tasks.id", ondelete="CASCADE"), index=True)
    workspace_id: Mapped[str] = mapped_column(
        ForeignKey("workspaces.id", ondelete="CASCADE"),
...
class FileOrganizationProposal(Base):
    __tablename__ = "file_organization_proposals"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    task_id: Mapped[str] = mapped_column(ForeignKey("tasks.id", ondelete="CASCADE"), index=True)
    workspace_id: Mapped[str] = mapped_column(
        ForeignKey("workspaces.id", ondelete="CASCADE"),
...
class FileRecycleProposal(Base):
    ...
    previous_file_status: Mapped[str] = mapped_column(String(32), nullable=False)
    error_message: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, nullable=False)
    confirmed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    recycled_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class RecoveryReconciliationProposal(Base):
    __tablename__ = "recovery_reconciliation_proposals"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    ...
    snapshot_state: Mapped[str] = mapped_column(String(64), nullable=False)
